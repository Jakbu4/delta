...
v = Canvas(root, width=540, height=400).pack()
...
def delta():
    ...
    dt_l = Label(v, text=f"\u0394 = {dt}", font="Helvetica 20 bold").place(x=225, y=190)
    ...
    if dt > 0:
        ...
        return dt_l, mz1_l, mz2_l
    elif dt == 0:
        mz1 = ((-1)*b) / (2*a)
        mz1_l = Label(v, text=f"x1 = {mz1}", font="Helvetica 15 bold").place(x=220, y=260)
        return dt_l, mz1_l
    elif dt < 0:
        mz0_l = Label(v, text="brak miejsc zerowych", font="Helvetica 15 bold").place(x=165, y=260)
        return dt_l, mz0_l
